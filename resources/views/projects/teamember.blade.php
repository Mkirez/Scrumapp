				...
				<table class="table">
					...
					@foreach($users_in_project as $users_in_projec)
						<tr>
							...
							<td>
								@if($project->users()->where('user_id', $users_in_projec->id)->first()->pivot->project_right != 2)
								<a href="{{url('/projects/'.$project->id. '/teamember/'.$users_in_projec->id .'/remove')}}">delete</a>
								@endif
							</td>
						</tr>
						@endforeach
